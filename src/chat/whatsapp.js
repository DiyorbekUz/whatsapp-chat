class Chat {
    ...
    async usersFunc(){
        try{
            const api = await fetch('https://whatsapp-chatt.herokuapp.com/users')
    ...
    async getMessages(){
        try{
            const api = await fetch('https://whatsapp-chatt.herokuapp.com/messages')
            const json = await api.json()
            return Promise.resolve(json)
        }catch(error){
            return Promise.reject(error)
    ...
    async getWriteUsers(){
        try{
            const api = await fetch('https://whatsapp-chatt.herokuapp.com/writeusers')
            const json = await api.json()
            return Promise.resolve(json)
        }catch(error){
            return Promise.reject(error)
    ...
    async sendMessage(fromUsername, toUsername, comment, date, photo_url, hidingDate){
        ...
        let api = await fetch('https://whatsapp-chatt.herokuapp.com/sendmessage', {
                method: 'POST',
                body: JSON.stringify({
                    who: whoUser,
    ...
    async headingUsers(who, username, photo_url, date){
        let api = await fetch('https://whatsapp-chatt.herokuapp.com/addwriteusers', {
                method: 'POST',
                body: JSON.stringify({
                    who: who,
                    username: username,
        ...
        if (api) {
            let json1 = await fetch('https://whatsapp-chatt.herokuapp.com/addwriteusers', {
                method: 'POST',
                body: JSON.stringify({
                    who: username,
                    username: who,
